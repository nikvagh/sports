  ...
  const updatePasswordValidation = (formData) => {
    $(".validation-popup").css('display','none');
    var call = ajaxCall('/'+ADMIN+'/change-password/validation', 'post', 'json', formData, []);
    userAuth(call);
    if (call.status == 200) {
      let response = call.responseJSON;
  ...
  const updatePassword = (id) => {
    ...
    btn_disable(true,'save_btn','validating, Please Wait...');
    var formData1 = new FormData(document.getElementById('form'));
    if (updatePasswordValidation(formData1)) {
      btn_disable(true,'save_btn','saving, Please Wait...');
      var formData2 = formData1;
      
      var call = ajaxCall('/'+ADMIN+'/change-password/'+id, 'post', 'json', formData2, []);
      userAuth(call);
      if (call.status == 200) {
