              ...
              <div class="form-group mb-3">
                ...
                <select class="select2 form-control" name="application" id="application">
                  ...
                  @foreach($applications as $key=>$val)
                    <option value="{{ $val->id }}" {{ ($val->id == $row->application_id) ? 'selected' : '' }}>&nbsp;&nbsp; {{ $val->name }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="col-sm-6">
              <div class="row">
                ...
                <div class="col-sm-2 d-flex align-items-center">
                  <img src="{{ ($row->wallpaperUrl !='') ? $row->wallpaperUrl : url(back_asset('images/no-img.png')) }}" alt="" class="img-fluid" id="wallpaper_img">
                </div>
              </div>
            </div>
...
<script src="{{ back_asset('js/custom/applicationWallpaper.js') }}"></script>
